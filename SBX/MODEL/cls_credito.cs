...
namespace SBX.MODEL
{
    public class cls_credito
    {
        ...
        public DataTable mtd_consultar_credito_exacto()
        {
            v_query = " SELECT "+
   ...
   "  ORDER BY Codigo DESC";
        ...
        public DataTable mtd_consultar_productos()
        {
            v_query = " SELECT CONCAT('Cant:',v.Cantidad,' - ',p.Item,' - ',p.Referencia,' - ',p.CodigoBarras, ' - ',p.Nombre) Producto FROM Venta v "+
                      " INNER JOIN Producto p ON p.item = v.Producto "+
                      " WHERE v.Credito = "+Codigo;
            v_dt = cls_datos.mtd_consultar(v_query);
            return v_dt;
        }
        public DataTable mtd_consultar_abono_sistema()
        {
            v_query = " SELECT ISNULL(SUM(ValorAbono),0) Pago,COUNT(*)+1 Cuotas FROM AbonoCredito "+
        ...
        {
            v_query = "SELECT Credito,Fecha,ValorAbono FROM AbonoCredito WHERE Credito = " + Codigo;
            v_dt = cls_datos.mtd_consultar(v_query);
            return v_dt;
        }
        ...
        {
            v_query = " INSERT INTO Credito (Fecha,Valor,AbonoInicial,PeriodoPago,Suministrar,NumCuotas,"+
        ...
        {
            v_query = " INSERT INTO AbonoCredito (Fecha,ValorAbono,Credito,usuario)" +
                      " VALUES (@Fecha,@ValorAbono,@Credito,@usuario)";
        ...
        {
            v_query = "SELECT Codigo FROM Venta WHERE Credito = '" + Codigo + "'";
            v_dt = cls_datos.mtd_consultar(v_query);

            v_query = "DELETE FROM Credito WHERE Codigo = '" + Codigo + "'";
